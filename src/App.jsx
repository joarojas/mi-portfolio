...
import { ME, PROJECTS, SKILLS, EXPERIENCE, CERTIFICATES, TERMINAL_COMMANDS } from "./data";
...
// ── Helper: lee un campo que puede ser string o {es, en} ──
const tx = (field, lang) => {
  if (!field) return "";
  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    textarea:-webkit-autofill, textarea:-webkit-autofill:hover, textarea:-webkit-autofill:focus {
      -webkit-box-shadow: 0 0 0px 1000px #0b0b11 inset !important;
      -webkit-text-fill-color: #f0f0f0 !important;
      caret-color: #ff3cac !important;
      transition: background-color 5000s ease-in-out 0s;
    }
    .lang-dropdown { position:relative; }
    .lang-menu {
      position:absolute; top:calc(100% + 8px); right:0;
      background:#0d0d16; border:1px solid rgba(255,60,172,.25);
      border-radius:12px; overflow:hidden; z-index:200;
      min-width:140px; animation:fadeIn .15s ease;
      box-shadow:0 8px 32px rgba(0,0,0,.4);
    }
    .lang-option {
      display:flex; align-items:center; gap:8px;
      padding:.6rem 1rem; cursor:none;
      font-family:'JetBrains Mono',monospace; font-size:.75rem;
      color:#888; letter-spacing:.06em;
      transition:background .15s, color .15s;
      border:none; background:transparent; width:100%; text-align:left;
    }
    .lang-option:hover { background:rgba(255,60,172,.1); color:#f0f0f0; }
    .lang-option.active { color:#ff3cac; }
    .lang-trigger {
      display:flex; align-items:center; gap:6px;
      padding:.4rem .9rem; border-radius:999px;
      background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.1);
      color:#888; font-family:'JetBrains Mono',monospace; font-size:.72rem;
      cursor:none; transition:all .2s; letter-spacing:.06em;
    }
    .lang-trigger:hover { border-color:rgba(255,60,172,.4); color:#f0f0f0; }
    .lang-trigger.open  { border-color:rgba(255,60,172,.4); color:#ff3cac; }
    /* Cert cards */
    .cert-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:1.25rem; }
    .cert-card { background:#0b0b11; border:1px solid rgba(255,255,255,.06); border-radius:20px; padding:1.5rem; transition:all .3s; display:flex; flex-direction:column; gap:10px; }
    .cert-card:hover { transform:translateY(-5px); border-color:rgba(255,60,172,.3); box-shadow:0 20px 60px rgba(255,60,172,.08); }
  `}</style>
);

/* ── HELPER: leer campo bilingüe ── */
// Ya definido arriba como tx()
        ...
        <div className="lang-menu">
          {LANGUAGES.map(l => (
            <button key={l.code} className={`lang-option${lang === l.code ? " active" : ""}`}
              ...
              {lang === l.code && <span style={{ marginLeft: "auto", color: "#ff3cac" }}>✓</span>}
            </button>
          ))}
        </div>
      )}
...
function Starfield() {
  ...
  useEffect(() => {
    ...
    const stars = Array.from({ length: 220 }, () => ({ x: Math.random()*W, y: Math.random()*H, r: Math.random()*1.2+.2, t: Math.random()*Math.PI*2, s: Math.random()*.012+.004 }));
    let raf;
    const draw = () => {
...
/* ── GITHUB STATS ── */
function GitHubStats({ username, t }) {
  ...
  const statsData = data
    ? [{val:data.public_repos,label:"REPOS",color:"#ff3cac"},{val:data.followers,label:"FOLLOWERS",color:"#2de2e6"},{val:data.following,label:"FOLLOWING",color:"#f6f740"}]
    : [{val:"4+",label:"REPOS",color:"#ff3cac"},{val:"3",label:"FOLLOWERS",color:"#2de2e6"},{val:"5",label:"FOLLOWING",color:"#f6f740"}];
  return (
...
/* ── TERMINAL ── */
function Terminal({ onClose, t }) {
  const [lines,setLines]=useState([{t:"cmd",v:t.terminal.welcome},{t:"muted",v:t.terminal.hint}]);
  ...
  const run=cmd=>{
    ...
    if(c==="clear"){setLines([{t:"muted",v:t.terminal.cleared}]);setInput("");return;}
    if(!c){setLines(p=>[...p,...newLines]);setInput("");return;}
    const fn=TERMINAL_COMMANDS[c];
    const result=fn?fn():[{t:"error",v:t.terminal.notFound.replace("%s",c)}];
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(0,0,0,.85)", backdropFilter:"blur(12px)" }}
      ...
      <div style={{ width:"min(760px,95vw)", maxHeight:"80vh", background:"#08080f", border:"1px solid rgba(255,60,172,.3)", borderRadius:20, overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 0 80px rgba(255,60,172,.15)", animation:"slideDown .3s ease" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", padding:".85rem 1.5rem", borderTop:"1px solid rgba(255,255,255,.05)", background:"#0d0d16", gap:8 }}>
          ...
          <input ref={inputRef} className="term-input" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")run(input);}} placeholder={t.terminal.placeholder}/>
...
/* ── SKILL CARD ── */
function SkillCard({ sk, i, t }) {
  // Traduce el nombre de la categoría si existe en translations.skills.cats
  const catName = (t.skills.cats && t.skills.cats[sk.cat]) ? t.skills.cats[sk.cat] : sk.cat;
  return (
    <Reveal delay={i*70}>
      <div className="skill-card" style={{ background:"#0b0b11", border:"1px solid rgba(255,255,255,.05)", borderRadius:20, padding:"1.5rem", transition:"border-color .3s,transform .3s" }}
        ...
        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1rem", color:sk.color, letterSpacing:".1em", marginBottom:"1.1rem" }}>{catName}</div>
    ...
    <Reveal delay={i*80}>
      <div className="cert-card">
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".65rem", color:cert.color||"#ff3cac", letterSpacing:".1em" }}>{tx(cert.issuer,t.__lang)}</span>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".6rem", color:"#444" }}>{tx(cert.date,t.__lang)}</span>
        </div>
        <h3 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1.3rem", color:"#f0f0f0", letterSpacing:".04em", lineHeight:1.1 }}>{tx(cert.title,t.__lang)}</h3>
        {cert.url && (
...
export default function Portfolio() {
  ...
  const handleSubmit=async e=>{
    ...
    try{
      await emailjs.send(EMAILJS_SERVICE,EMAILJS_TEMPLATE,{from_name:formData.name,from_email:formData.email,message:formData.message});
      setFormStatus("sent");setFormData({name:"",email:"",message:""});
      setTimeout(()=>setFormStatus("idle"),4000);
    }catch(err){
      console.error("EmailJS error:",err);setFormStatus("error");
      setTimeout(()=>setFormStatus("idle"),4000);
    }
  };
  ...
  useEffect(()=>{
    const ids=["hero","about","skills","projects","experience","certificates","contact"];
    ...
    <div style={{ fontFamily:"'Cabinet Grotesk',sans-serif", background:"#050508", color:"#f0f0f0", minHeight:"100vh" }} onMouseMove={handleMouse}>
      ...
      <nav className="main-nav">
        ...
        <ul className="nav-links">
          {NAV_ITEMS.map(([lbl,href],i)=>(
            <li key={href}><a href={href} className={`nav-a${activeNav===i?" active":""}`}>{lbl}</a></li>
          ))}
          <li><button onClick={()=>setShowTerm(true)} className="nav-term-btn">{t.nav.terminal}</button></li>
          <li><LangDropdown lang={lang} setLang={setLang}/></li>
          <li><a href={ME.cv} download className="nav-cv-btn">{t.nav.cv}</a></li>
        </ul>
      </nav>
      ...
      <section id="hero" className="hero-section">
        ...
        <div className="hero-grid">
          <div className="hero-text-col">
            ...
            <div className="hero-btns">
              <MagBtn href="#projects" bg="#ff3cac" color="#000" glow="0 0 40px rgba(255,60,172,.4)">{t.hero.btnProjects}</MagBtn>
              <MagBtn href="#contact"  bg="transparent" color="#f0f0f0">{t.hero.btnTalk}</MagBtn>
              <MagBtn href={ME.cv}     bg="rgba(45,226,230,.08)" color="#2de2e6" download>{t.hero.btnCv}</MagBtn>
            </div>
        ...
        <div className="hero-scroll"><span>SCROLL</span><div/></div>
      </section>
        ...
        <div className="section-container">
          ...
          <div className="about-grid">
            <div>
              ...
              <Reveal delay={80}><p style={{ color:"#555", lineHeight:1.85, marginBottom:"1.2rem", fontWeight:300, fontSize:"1rem" }}>{t.about.p1}</p></Reveal>
              <Reveal delay={150}><p style={{ color:"#555", lineHeight:1.85, marginBottom:"1.2rem", fontWeight:300, fontSize:"1rem" }}>{t.about.p2}</p></Reveal>
              <Reveal delay={220}><p style={{ color:"#555", lineHeight:1.85, fontWeight:300, fontSize:"1rem" }}>{t.about.p3}</p></Reveal>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
              <Reveal delay={100}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem" }}>
                  {t.about.stats.map((s,i)=>{
                    const colors=["#ff3cac","#2de2e6","#f6f740","#4ade80"];
                    return (
                      <div key={i} style={{ background:"#0b0b11", border:"1px solid rgba(255,255,255,.05)", borderRadius:16, padding:"1.25rem" }}>
                        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"2.8rem", color:colors[i], lineHeight:1 }}>{s.n}</div>
              ...
              <Reveal delay={200}><GitHubStats username={ME.github} t={t}/></Reveal>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="skills" style={{ background:"#050508" }}>
        <div className="section-container">
          ...
          <div className="skills-grid">
            {SKILLS.map((sk,i)=><SkillCard key={sk.cat} sk={sk} i={i} t={t}/>)}
          </div>
        </div>
      </section>
      ...
      <section id="projects" style={{ background:"#07070c", borderTop:"1px solid rgba(255,255,255,.03)" }}>
        <div className="section-container">
          ...
          <div className="projects-grid">
            {PROJECTS.map((p,i)=><ProjCard key={p.id} p={p} i={i} lang={lang} t={t}/>)}
            <Reveal delay={320}>
            ...
            {EXPERIENCE.map((item,i)=>(
              <Reveal key={i} delay={i*100}>
                <div style={{ position:"relative", marginBottom:"2.5rem", paddingLeft:"1rem" }}>
                  ...
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".68rem", color:"#444", letterSpacing:".07em", marginBottom:6 }}>{tx(item.date,lang)}</div>
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1.5rem", color:"#f0f0f0", letterSpacing:".04em", marginBottom:4 }}>{tx(item.title,lang)}</div>
                  <div style={{ color:item.color, fontSize:".88rem", marginBottom:8, fontWeight:500 }}>{tx(item.subtitle,lang)}</div>
                  <p style={{ color:"#555", fontSize:".88rem", lineHeight:1.7, fontWeight:300 }}>{tx(item.desc,lang)}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* CERTIFICATES — solo se muestra si hay datos */}
      {hasCerts && (
        <section id="certificates" style={{ background:"#07070c", borderTop:"1px solid rgba(255,255,255,.03)" }}>
          <div className="section-container">
            ...
            <div className="cert-grid">
              {CERTIFICATES.map((cert,i)=><CertCard key={i} cert={cert} i={i} t={t}/>)}
            </div>
          </div>
        </section>
      )}
      ...
      <section id="contact" style={{ background: hasCerts ? "#050508" : "#07070c", borderTop:"1px solid rgba(255,255,255,.03)" }}>
        <div className="section-container">
          <Reveal>{sLabel(hasCerts?6:5,t.contact.label)}</Reveal>
          <div className="contact-grid">
            <div>
              <Reveal>{sH2(t.contact.title)}</Reveal>
              <Reveal delay={80}><p style={{ color:"#555", lineHeight:1.85, fontWeight:300, marginBottom:"2.5rem" }}>{t.contact.desc}</p></Reveal>
            ...
            <Reveal delay={150}>
              <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
                ...
                <button type="submit" disabled={formStatus==="sending"}
                  style={{ alignSelf:"center", padding:".8rem 2rem", borderRadius:999, background:formStatus==="sent"?"#4ade80":formStatus==="error"?"#ff6b6b":formStatus==="sending"?"rgba(255,60,172,.4)":"linear-gradient(135deg,#ff3cac,#a855f7)", color:(formStatus==="sent"||formStatus==="error")?"#000":"#fff", fontFamily:"'Cabinet Grotesk',sans-serif", fontWeight:800, fontSize:".9rem", border:"none", cursor:formStatus==="sending"?"wait":"none", transition:"all .4s", opacity:formStatus==="sending"?.7:1, boxShadow:formStatus==="sent"?"0 0 30px rgba(74,222,128,.4)":"0 0 30px rgba(255,60,172,.3)" }}>
                  {formStatus==="sending"?t.contact.btnSending:formStatus==="sent"?t.contact.btnSent:formStatus==="error"?t.contact.btnError:t.contact.btnIdle}
                </button>
              </form>
            </Reveal>
          </div>
      ...
      <footer style={{ borderTop:"1px solid rgba(255,255,255,.04)", padding:"1.75rem 3rem", display:"flex", alignItems:"center", justifyContent:"space-between", color:"#333", fontSize:".78rem" }}>
        ...
        <span>{t.footer.made} {ME.location}</span>
